...
@section('title')
    Edit book {{ $book->title}} by {{ $book->author->first_name }} {{ $book->author->last_name }}
@endsection

@section('content')

@if(session('flash-alert'))  
<div><p class="flash-alert">
{{ session('flash-alert')}}
</p></div>
@endif
...
<h2>{{ $book->title}}</h2>
    ...
    <p>Want to change a book listing? - you can edit it here!</p>
    ...
    <form method='POST' action='/books/{{$book->slug}}'>
        ...
        {{-- includes are subviews. See Blade cheat sheet in Notes --}}

        <label for='author_id'>* Author</label>
        <select name='author_id'>
            <option value=''>Choose one...</option>
            @foreach($authors as $author)
                <option value='{{ $author->id }}'  {{ ( $author->id == $book->author->id || old('author_id') == $author->id) ? 'selected' : '' }}>{{ $author->first_name.' '.$author->last_name }}</option>
            @endforeach
        </select>
